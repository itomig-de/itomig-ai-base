...
class OpenAIEmbeddingEngine extends GenericEmbeddingEngine implements iEmbeddingEngineInterface
{
	...
	public function GetEmbeddingGenerator(): EmbeddingGeneratorInterface
	{
		...
		$config->apiKey = $this->sApiKey;
		if (!empty($this->sUrl)) {
			$config->url = $this->sUrl;
		}
		$config->model = $this->sModel;

		return match ($this->sModel) {
			...
			default => new OpenAICompatibleGenerator($config, $this->sModel, $this->iDimensions),
		};
	}
}